...
#pragma HLS UNROLL
    ...
    //Processs image
    if (pX == 0)
      nnet::reset_down<data_T, data_T, CONFIG_T>(pX, layer_in_row, layer_in); //check stride
...
#pragma HLS UNROLL
    ...
    if (pX == lShiftX && pY > lShiftY - 1)
      pPass = true;
  }

template <class data_T, class res_T, typename CONFIG_T>
  void pooling2d_cl_arr(
      hls::stream<data_T> data[CONFIG_T::n_filt_in],
      hls::stream<res_T> res[CONFIG_T::n_filt_in])
  {
  ...
  #pragma HLS PIPELINE
    ...
    for (int i0 = 0; i0 < pLoop; i0++)
    {
      ...
      if ((pX + 1) % CONFIG_T::stride_width == 0 && (pY + 1) % CONFIG_T::stride_height == 0 && pY > lShiftY - 1 && pX > lShiftX - 1)
      {
        ...
        PoolFilt: for (unsigned i1 = 0; i1 < CONFIG_T::n_filt; i1++)
        {
          #pragma HLS UNROLL
          data_T pool[CONFIG_T::pool_height * CONFIG_T::pool_width];
          #pragma HLS ARRAY_RESHAPE variable = pool complete dim = 0
          PoolWin: for (unsigned i2 = 0; i2 < CONFIG_T::pool_height * CONFIG_T::pool_width; i2++)
          {
            #pragma HLS UNROLL
  ...
  #pragma HLS PIPELINE
    ...
    const static unsigned depth = CONFIG_T::in_height*CONFIG_T::in_width;
    hls::stream<data_T> tmpdata[CONFIG_T::n_chan];
    #pragma HLS STREAM variable = tmpdata depth = depth dim = 1
...
#pragma HLS UNROLL
      ...
      hls::stream<res_T> res[CONFIG_T::n_filt_in])
  {
    ...
    // while (!padded_data[0].empty())
    // {
    //   pooling2d_cl_nopad_pad<data_T, res_T, CONFIG_T>(padded_data, res);
    // }
    ...
    {
      pooling2d_cl_padded<data_T, res_T, CONFIG_T>(padded_data, res);
    }
  }
...
#pragma HLS UNROLL
                    ...
                    data_T res[CONFIG_T::out_height * CONFIG_T::out_width * CONFIG_T::n_filt])
  {
    ...
    PoolInputFilt: for (int ff = 0; ff < CONFIG_T::n_filt; ff++)
    {
      // Loop over input image y in steps of stride
      PoolInputHeight: for (int ii = 0; ii < padded_height; ii += CONFIG_T::stride_height)
      {
        // Loop over input image x in steps of stride
        PoolInputWidth: for (int jj = 0; jj < padded_width; jj += CONFIG_T::stride_width)
        {
          data_T pool[CONFIG_T::pool_height * CONFIG_T::pool_width];
          // Keep track of number of pixels in image vs padding region
          unsigned img_overlap = 0;
          // Loop over pool window y
          PoolWinHeight: for (int kk = 0; kk < CONFIG_T::stride_height; kk++)
          {
            // Loop over pool window x
            PoolWinWidth: for (int ll = 0; ll < CONFIG_T::stride_width; ll++)
            {
