...
#pragma HLS ARRAY_RESHAPE variable = tmpinput complete dim = 0
  ...
  void shift_right_small( //To be fixed with stride
      ...
      res_T data[CONFIG_T::filt_width * CONFIG_T::filt_height * CONFIG_T::n_chan])
  {
  // Read new input into shift registers, and pop into filter
  // "shift register" for the filter
...
#pragma HLS inline
    ...
    {
#pragma HLS PIPELINE II = 1
    // data (filter) shift the filter over by a column
    for (unsigned i1 = 0; i1 < CONFIG_T::filt_height; i1++)
      {
        for (unsigned i2 = 0; i2 < CONFIG_T::n_chan; i2++)
        {
...
#pragma HLS UNROLL
    ...
    if (pX == lShiftX && pPass)
      nnet::reset_down<data_T, data_T, CONFIG_T>(pY, layer_in_row, layer_in);
  ...
  // Reads data (by channel) into end of shift register and pops the front off.
  template <class data_T, class res_T, typename CONFIG_T>
  void cnnshift_arr(data_T data[CONFIG_T::n_chan], // Data is input we read
...
#pragma HLS PIPELINE
    ...
    // Temporary vector (slice of image not in shift register)
    data_T tmpinput[CONFIG_T::filt_height][CONFIG_T::n_chan];
#pragma HLS ARRAY_RESHAPE variable = tmpinput complete dim = 0

    ChannelLoop: for (int i0 = 0; i0 < CONFIG_T::n_chan; i0++)
    {
#pragma HLS UNROLL
      // Shift everything in shift register
      tmpinput[CONFIG_T::filt_height - 1][i0] = data[i0];

      FilterHeightLoop: for (unsigned i1 = 1; i1 < CONFIG_T::filt_height; i1++)
      {
#pragma HLS UNROLL
        // tmp1 new data
        data_T tmp1 = tmpinput[CONFIG_T::filt_height - i1][i0];
        // shifted in, tmp popped out of shift register
  ...
  void cnnshiftzero_arr(
      ...
      data_T output[(CONFIG_T::filt_height * CONFIG_T::filt_width) * (CONFIG_T::n_chan)])
  {
    // Assumes inputs are 0s and add stuff
#pragma HLS inline // Needs this to work, "include" for functions
    const static int rowsize = (CONFIG_T::in_width + CONFIG_T::pad_left + CONFIG_T::pad_right);
...
#pragma HLS UNROLL
      ...
      typename CONFIG_T::bias_t biases[CONFIG_T::n_filt])
  {
    ...
    const static int lShiftX = CONFIG_T::filt_width - CONFIG_T::pad_left - 1; // Threshold X for filter
    const static int lShiftY = CONFIG_T::filt_height - CONFIG_T::pad_top - 1; // Threshold Y for filter
    const static int rowsize = (CONFIG_T::in_width + CONFIG_T::pad_left + CONFIG_T::pad_right); // Total width
...
#pragma HLS ARRAY_RESHAPE variable = layer_reluout complete dim = 0
    ...
    #pragma HLS ARRAY_RESHAPE variable = layer_out complete dim = 0
    static int pX = 0; // Pointer X
    static int pY = 0; // Pointer Y

    data_T iReset = data[0].read(); // Read channel 0 for 1 (new image), 0 (same image)

    // Read data in temporary buffer
    ...
    // pLoop does zero padding, (1 if not padded) (!= 1 adds zeros)
    unsigned pLoop = 1;
    if (pX == CONFIG_T::in_width - 1) // End of line
      pLoop = CONFIG_T::pad_right + 1;
    if (pX == CONFIG_T::in_width - 1 && pY == CONFIG_T::in_height - 1) // End of line + end of image
    ...
    {
      if (i0 > 0) // Adds zero padding
        nnet::cnnshiftzero_arr<data_T, res_T, CONFIG_T>(layer_in_row, layer_in);

      // pY > lShiftY - 1 && pX > lShiftX - 1  Start for the first time
      // (pX - lShiftX) % CONFIG_T::stride_width == 0 && (pY - lShiftY) % CONFIG_T::stride_height == 0 everywhere else
      if ((pX - lShiftX) % CONFIG_T::stride_width == 0 && (pY - lShiftY) % CONFIG_T::stride_height == 0 && pY > lShiftY - 1 && pX > lShiftX - 1)
      {
        ...
        nnet::relu<res_T,res_T,typename CONFIG_T::relu_config>(layer_out, layer_reluout); // TODO: fix activations
        nnet::fill_image<data_T, data_T, CONFIG_T>(layer_reluout, pReset, res);
        if (pReset == 0)
          pReset = 1; // Once you've filled output array for first time, signal (to next layer) to start
      }

      // Pointer housekeeping
      pX = pX + 1;
      if (pX == CONFIG_T::in_width + CONFIG_T::pad_right) // end of line after padding 
      {
        pX = 0;
        pY = pY + 1;

        // End of line add padding
        for (unsigned iY = 0; iY < CONFIG_T::pad_left; iY++)
          nnet::cnnshiftzero_arr<data_T, res_T, CONFIG_T>(layer_in_row, layer_in); // Adds left side zeros
      }
    }
  }

  // Assumes there's padding
...
#pragma HLS UNROLL
    ...
    static res_T pReset = 0;
    if (tmpdata[0] == 0) // Start of an image
    {
      pX = 0; // Reset pointers
      pY = 0;
      pReset = 0;
...
#pragma HLS UNROLL
    ...
    iN++;
    unsigned pLoop = 0;
    if (pX == CONFIG_T::in_width - 1) // End of row, add pad_right + pad_left zeros
...
#pragma HLS UNROLL
  ...
  // No padding, pre-layer that does the padding
  template <class data_T, class res_T, typename CONFIG_T>
...
#pragma HLS STREAM variable = tmpdata depth = ntotal dim = 2
    ...
    for (int i0 = 0; i0 < CONFIG_T::in_width + CONFIG_T::pad_left + CONFIG_T::pad_right; i0++)
    {
      ...
      if (pIndex > CONFIG_T::n_split - 1)
        pIndex = CONFIG_T::n_split - 1;
...
#pragma HLS UNROLL
          ...
          if (i0 % nrange < CONFIG_T::filt_width && i0 > nrange)
            tmpdata[pIndex - 1][i2].write(pData);
        }
      }
    }
